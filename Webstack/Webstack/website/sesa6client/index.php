...
<body>
	...
	<div class="container">
			<div class="left-container">
				<a href="index.php"><div class="element logo" id="home">
					<div class="brandlog">
...
function readparsing(){
	...
	else if(data.indexOf('sort') !== -1&&data.indexOf('ascending') !== -1){
		if(data.indexOf('icon') !== -1){
		    audio2.play();
            setCookie("iconvehtypevar","order",1);
        }
        else if(data.indexOf('owner name') !== -1){
            audio2.play();
            setCookie("ownernamevar","order",1);
        }
        else if(data.indexOf('vehicle ID') !== -1){
            audio2.play();
            setCookie("vehidvar","order",1);
        }
        else if(data.indexOf('severity') !== -1){
            audio2.play();
            setCookie("sevvar","order",1);
        }
        else if(data.indexOf('timestamp') !== -1){
            audio2.play();
            setCookie("tsvar","order",1);
        }
        else if(data.indexOf('department contacted') !== -1){
            audio2.play();
            setCookie("depvar","order",1);
        }
        else if(data.indexOf('location') !== -1){
            audio2.play();
            setCookie("locationvar","order",1);
        }
        else if(data.indexOf('device ID') !== -1){
            audio2.play();
            setCookie("deviceidvar","order",1);
        }
        else{
            speecherrorc();
        }
	}
	else if(data.indexOf('sort') !== -1&&data.indexOf('descending') !== -1){
		if(data.indexOf('icon') !== -1){
		    audio2.play();
	...
	else if(data.indexOf('sort') !== -1&&data.indexOf('remove') !== -1){
		if(data.indexOf('icon') !== -1){
		    audio2.play();
            setCookie("iconvehtypevar","order",0);
        }
        else if(data.indexOf('owner name') !== -1){
            audio2.play();
            setCookie("ownernamevar","order",0);
        }
        else if(data.indexOf('vehicle ID') !== -1){
            audio2.play();
            setCookie("vehidvar","order",0);
        }
        else if(data.indexOf('severity') !== -1){
            audio2.play();
            setCookie("sevvar","order",0);
        }
        else if(data.indexOf('timestamp') !== -1){
            audio2.play();
            setCookie("tsvar","order",0);
        }
        else if(data.indexOf('department contacted') !== -1){
            audio2.play();
            setCookie("depvar","order",0);
        }
        else if(data.indexOf('location') !== -1){
            audio2.play();
            setCookie("locationvar","order",0);
        }
        else if(data.indexOf('device ID') !== -1){
            audio2.play();
            setCookie("deviceidvar","order",0);
        }
        else{
            speecherrorc();
        }
		...
		function changefil(divid){
			if(divid == "icon-veh-type"){
				...
				if(tsvar == 0){
					...
					tsvar  = 2;
					setCookie("tsvar","Timestamp_date DESC,Timestamp_time DESC",2)
				}
				else{
					document.getElementById("uaiconts").style.display="none";
					...
					document.getElementById("uats").style.display="none";
					tsvar  = 0;
					setCookie("tsvar","Timestamp_date ASC,Timestamp_time ASC",2)
				}
			}
			else if(divid == "dep"){
				if(depvar == 0){
	...
	function texts(message){
		...
		setInterval(() => {
			if(message!=0){
				if(aubw==1){
					audio.play();
					texts("Emergency. " + message + " casualties need to be rescued");
				}
			}
		},30000);

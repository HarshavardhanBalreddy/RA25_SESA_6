...
if($sortfilter==""){
	$sql = "SELECT * FROM accidentrecord WHERE Timestamp_date='".date("Y-m-d")."' AND ".$Depl."_ID='".$DepartmentID."'";	
}
else{
	$sql = "SELECT * FROM accidentrecord WHERE Timestamp_date='".date("Y-m-d")."' AND ".$Depl."_ID='".$DepartmentID."' ORDER BY ".$sortfilter;
}
						...
						echo '<div class="tansbg">
								<form method="POST" action="tranfpage.php">
                					<div class="transbg transb">
                    						...
                    						<button name="transb" id="t'.$DeviceID.'">Transfer</button>
                					</div>
                				</form>
                					<div class="transbg acceb">
                    						<button name="acceb" id="a'.$DeviceID.'" onclick="transb(\''.$DeviceID.' '.$Timestamp_date.' '.$Timestamp_time.' '.$Depl.'\');">Accept</button>
                					</div>
    						</div>';
					}
					else if($Status==1){
